...
const Hero = () => {
    ...
    return (
        <section id="home" className="min-h-screen flex flex-col justify-center items-center relative text-white text-center p-4 overflow-hidden bg-zan-dark">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] border border-zan-cyan/10 rounded-full animate-spin-slow z-0 pointer-events-none"></div>
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] border border-zan-red/10 rounded-full animate-spin-slow animation-delay-2000 z-0 pointer-events-none"></div>

            {/* Main Title Content */}
            <div className="relative z-10 w-full max-w-5xl mx-auto flex flex-col items-center justify-center min-h-[50vh]">
                <div className="space-y-8">
                    {/* Glitch Effect Title Container */}
                    <div className="relative">
                        <h1 className="text-5xl md:text-7xl lg:text-8xl font-extrabold leading-tight tracking-tight font-heading">
                            <span className="block text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">{firstLine}</span>
                            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-zan-cyan via-white to-zan-cyan animate-shine bg-[length:200%_auto]">
                                {secondLine}
                            </span>
                        </h1>
                    </div>

                    <p className="max-w-2xl mx-auto text-lg md:text-xl text-gray-300 font-light tracking-wide border-l-2 border-zan-red pl-4 text-left">
                    ...
                    {/* CTA Button */}
                    <div className="pt-8">
                        <a href="#showcase" className="relative px-8 py-3 bg-transparent border border-zan-cyan text-zan-cyan font-bold uppercase tracking-widest hover:bg-zan-cyan hover:text-black transition-all duration-300 group overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 md:pb-20 mt-12">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                    {stats.map((stat, index) => (
                        <div key={index} className="group relative p-6 bg-surface-dark/40 backdrop-blur-md border border-white/10 hover:border-zan-cyan/50 transition-all duration-300 text-center overflow-hidden">
                            {/* Card Hover Glow */}
                            ...
                            <h3 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-2 group-hover:text-zan-cyan transition-colors font-heading inline-block min-w-[3ch]">
