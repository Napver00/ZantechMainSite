    ...
    <div className="group h-full flex flex-col">
        <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-lg rounded-2xl p-6 border border-white/20 dark:border-gray-700/20 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 text-center flex-grow flex flex-col">
            ...
            <div className="flex-grow flex flex-col">
                ...
                <p className="text-cyan-600 dark:text-cyan-400 font-semibold mb-4">{ambassador.campus}</p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg p-8 relative">
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                    <div>
                        ...
                        <input name="fullName" type="text" placeholder="Your Name" className="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 border-transparent rounded-lg focus:ring-2 focus:ring-cyan-500 transition-all" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Email Address</label>
                        <input name="email" type="email" placeholder="your.email@university.com" className="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 border-transparent rounded-lg focus:ring-2 focus:ring-cyan-500 transition-all" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">University/Campus</label>
                        <input name="campus" type="text" placeholder="Your University" className="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 border-transparent rounded-lg focus:ring-2 focus:ring-cyan-500 transition-all" required />
                    </div>
                    <div>
                            ...
                            <div className="space-y-1 text-center">
                                ...
                                <div className="flex text-sm text-gray-600 dark:text-gray-400">
                                    <label htmlFor="file-upload" className="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-cyan-600 hover:text-cyan-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-cyan-500">
                                        ...
                                        <input id="file-upload" name="photo" type="file" className="sr-only" onChange={handleFileChange} accept="image/*" required />
                                    </label>
                                    <p className="pl-1">or drag and drop</p>
                                </div>
                                {fileName ? <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{fileName}</p> : <p className="text-xs text-gray-500 dark:text-gray-400">PNG, JPG up to 10MB</p>}
                            </div>
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Why do you want to join?</label>
                        <textarea name="reason" placeholder="Tell us about your passion for tech..." rows={4} className="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 border-transparent rounded-lg focus:ring-2 focus:ring-cyan-500 transition-all" required></textarea>
                    </div>
